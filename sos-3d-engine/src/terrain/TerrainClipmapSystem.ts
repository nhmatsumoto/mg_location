...
export class TerrainClipmapSystem {
  ...
  constructor(orchestrator: RenderOrchestrator, gisApi: any) {
  ...
  private async maybeFetchHeightmap(lat: number, lon: number) {
    ...
    const data = await this.gisApi.getElevationGrid(lat - delta, lon - delta, lat + delta, lon + delta, 128);
  ...
  private async maybeFetchIndicesData(lat: number, lon: number, type: string) {
    ...
    let data;
    if (type === 'ndvi') {
        data = await this.gisApi.getVegetationData(lat - delta, lon - delta, lat + delta, lon + delta);
    } else {
        data = await this.gisApi.getSoilData(lat - delta, lon - delta, lat + delta, lon + delta);
    }
